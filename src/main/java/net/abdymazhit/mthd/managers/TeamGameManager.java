...
public class TeamGameManager {
    ...
    /**
     * Инициализирует менеджер Team Rating игр
     */
    public TeamGameManager(GameManager gameManager) {
    ...
    public void finishGameTeam(int points, int games, int wins, int wonBeds, int lostBeds, int id) {
        try {
            Connection connection = MTHD.getInstance().database.getConnection();
            PreparedStatement playersStatement = connection.prepareStatement(
                    "UPDATE teams SET points = ?, games = games + ?, wins = wins + ?, won_beds = won_beds + ?, lost_beds = lost_beds + ? WHERE id = ?;");
