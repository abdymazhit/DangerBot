...
public record SingleGameManager(GameManager gameManager) {
    ...
    private void createGame(List<Integer> playersIds, int liveGameId, int firstTeamCaptainId, int secondTeamCaptainId, String format, int assistantId) {
        List<Category> categories = DangerBot.getInstance().guild.getCategoriesByName("Game-" + liveGameId, true);
        ...
        try {
            ...
            if(createResultSet.next()) {
                ...
                for(int playerId : firstTeamPlayersId) {
                    ...
                    if(winnerTeamId == 0) {
                        DangerBot.getInstance().gameManager.singleGameManager.finishGamePlayer(firstTeamRating, 1, 1, playerId);
                    } else {
                        DangerBot.getInstance().gameManager.singleGameManager.finishGamePlayer(firstTeamRating, 1, 0, playerId);
                    }
                }
                ...
                for(int playerId : secondTeamPlayersId) {
                    ...
                    if(winnerTeamId == 1) {
                        DangerBot.getInstance().gameManager.singleGameManager.finishGamePlayer(secondTeamRating, 1, 1, playerId);
                    } else {
                        DangerBot.getInstance().gameManager.singleGameManager.finishGamePlayer(secondTeamRating, 1, 0, playerId);
                    }
                }
            }
        } catch (SQLException e) {
    ...
    public void finishGamePlayer(int points, int games, int wins, int playerId) {
        try {
            ...
            playersStatement.executeUpdate();
            DangerBot.getInstance().playersChannel.updateTopMessage();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
