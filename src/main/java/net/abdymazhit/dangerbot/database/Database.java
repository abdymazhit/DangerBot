...
public class Database {
     ...
     */
    public Database() {
        Config.MySQL config = DangerBot.getInstance().config.mySQL;
...
//        Создать таблицы, только при необходимости
    ...
    public List<String> getSinglePlayersNames(int liveGameId, int teamId) {
        ...
        try {
            if(teamId == 0) {
                PreparedStatement captainStatement = DangerBot.getInstance().database.getConnection().prepareStatement("""
                    SELECT u.username as username FROM users as u
    ...
    public void banPlayer(@Nullable Integer bannerId, int playerId, int minutes) {
        try {
            ...
            ResultSet resultSet = preparedStatement.getGeneratedKeys();
            if(!resultSet.next()) {
                PreparedStatement updateStatement = DangerBot.getInstance().database.getConnection().prepareStatement("""
    ...
    public boolean deleteStream(int youtuberId, int deleterId) {
        try {
            ...
            if(resultSet.next()) {
                ...
                DangerBot.getInstance().liveStreamsManager.removeLiveStream(link);
            }
