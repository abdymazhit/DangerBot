...
public class AdminSingleAddCommandListener {
    ...
    public void onCommandReceived(MessageReceivedEvent event) {
        ...
        if(playerAccount.discordId != null) {
            DangerBot.getInstance().guild.addRoleToMember(playerAccount.discordId, UserRole.SINGLE_RATING.getRole()).queue();
        }

        message.reply("Игрок успешно добавлен в Single Rating! Имя игрока: %player%"
