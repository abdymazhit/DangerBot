...
public class AdminBanCommandListener {
    ...
    public void onCommandReceived(MessageReceivedEvent event) {
        ...
        int bannerId = DangerBot.getInstance().database.getUserId(member.getId());
        if(bannerId < 0) {
        ...
        UserAccount userAccount = DangerBot.getInstance().database.getUserAccount(playerName);
        if(userAccount == null) {
        ...
        try {
            ...
            DangerBot.getInstance().database.banPlayer(bannerId, userAccount.id, timeInMinutes);
