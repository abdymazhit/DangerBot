...
import net.abdymazhit.dangerbot.listeners.commands.team.TeamLeaveCommandListener;
...
public class AdminTeamDeleteCommandListener extends TeamLeaveCommandListener {
    ...
    public void onCommandReceived(MessageReceivedEvent event) {
        ...
        boolean isUserTeamLeader = DangerBot.getInstance().database.isUserTeamLeader(memberAccount.id);
        if(isUserTeamLeader) {
        ...
        boolean isUserTeamMember = DangerBot.getInstance().database.isUserTeamMember(memberAccount.id, teamId);
        if(!isUserTeamMember) {
