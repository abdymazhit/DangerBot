    ...
    private void choicePlayer(GameCategoryManager gameCategoryManager, MessageChannel messageChannel, Message message, Member member) {
        ...
        if(gameCategoryManager.playersChoiceChannel.channel.equals(messageChannel)) {
            ...
            if(contentRaw.startsWith("!add")) {
                ...
                int captainTeamId = DangerBot.getInstance().database.getUserTeamId(captainId);
                if(captainTeamId < 0) {
                ...
                int playerTeamId = DangerBot.getInstance().database.getUserTeamId(playerAccount.id);
                if(playerTeamId < 0) {
                ...
                if(member.getRoles().contains(UserRole.ASSISTANT.getRole())) {
                    ...
                    Category category = DangerBot.getInstance().guild.getCategoryById(gameCategoryManager.category.getId());
                    if(category == null) {
